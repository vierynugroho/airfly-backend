...
describe('AirportService', () => {
  ...
  describe('create', () => {
    it('should create an airport successfully', async () => {
      const mockFiles = { image: { name: 'airport.jpg', mimetype: 'image/jpeg' } };
      const mockData = { code: 'AP123', name: 'Test Airport' };
      const mockUploadResponse = { image: { url: 'https://image.url', fileId: '123' } };
      const mockCreatedAirport = { ...mockData, imageUrl: mockUploadResponse.image.url, imageId: mockUploadResponse.image.fileId };
      ...
      expect(UploadService.upload).toHaveBeenCalledWith(mockFiles, 'airports', ['airport']);
    ...
    it('should throw an error if no file is provided', async () => {
      ...
      await expect(AirportService.create({}, mockData)).rejects.toThrowError(new ErrorHandler(400, 'no file selected'));
    });
    ...
    it('should throw an error if airport code already exists', async () => {
      ...
      await expect(AirportService.create(mockFiles, mockData)).rejects.toThrowError(new ErrorHandler(409, 'Airport code already exists'));
    });
  });

  describe('update', () => {
    it('should update an airport successfully and upload a new image', async () => {
      const mockAirport = { id: 1, code: 'AP123', name: 'Test Airport', imageId: '123' };
      const mockFiles = { image: { name: 'updated.jpg', mimetype: 'image/jpeg' } };
      const mockData = { name: 'Updated Airport' };
      const mockUploadResponse = { image: { url: 'https://updated.url', fileId: '456' } };
      const mockUpdatedAirport = { ...mockAirport, ...mockData, imageUrl: mockUploadResponse.image.url, imageId: mockUploadResponse.image.fileId };
      ...
      const result = await AirportService.update(mockAirport.id, mockFiles, mockData);
    ...
    it('should update an airport successfully without uploading a new image', async () => {
      ...
      AirportRepository.findByID.mockResolvedValue(mockAirport);
      AirportRepository.update.mockResolvedValue({ ...mockAirport, ...mockData });
      ...
      expect(AirportRepository.update).toHaveBeenCalledWith(mockAirport.id, mockData);
    ...
    it('should throw an error if airport not found', async () => {
      ...
      await expect(AirportService.update(mockAirportID, mockFiles, mockData)).rejects.toThrowError(new ErrorHandler(404, 'Airport not found'));
    });
  });
  ...
  describe('delete', () => {
    ...
    it('should throw an error if airport not found', async () => {
      ...
      await expect(AirportService.delete(mockAirportID)).rejects.toThrowError(new ErrorHandler(404, 'Airport not found'));
    });
  });
  ...
  describe('findMany', () => {
    it('should retrieve airports with pagination', async () => {
      ...
      const result = await AirportService.findMany(mockPagination, mockFilter, mockSorter);

      expect(AirportRepository.findMany).toHaveBeenCalledWith(mockPagination, mockFilter, mockSorter);
  ...
  describe('findByID', () => {
    ...
    it('should throw an error if airport not found', async () => {
      ...
      await expect(AirportService.findByID(mockAirportID)).rejects.toThrowError(new ErrorHandler(404, 'Airport not found'));
    });
  });
  ...
  describe('findByCode', () => {
    it('should retrieve an airport by code', async () => {
      ...
      expect(AirportRepository.findByCode).toHaveBeenCalledWith(mockAirport.code);
      expect(result).toEqual(mockAirport);
    });
    ...
    it('should throw an error if airport not found', async () => {
      ...
      await expect(AirportService.findByCode(mockAirportCode)).rejects.toThrowError(new ErrorHandler(404, 'Airport not found'));
    });
  });
});
